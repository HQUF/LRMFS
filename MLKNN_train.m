...
%Computing distance between training instances
    ...
    if(num_training*num_training<max_mat_elements)
        ...
        %Computing Prior and PriorN  计算先验概率
        for i=1:num_class
        ...
        %Computing Cond and CondN  计算后验概率
        ...
        temp_Ci=zeros(num_class,Num+1); % 5*11 %The number of instances belong to the ith class which have k nearest neighbors in Ci is stored in temp_Ci(i,k+1)
        temp_NCi=zeros(num_class,Num+1); % 5*11 %The number of instances not belong to the ith class which have k nearest neighbors in Ci is stored in temp_NCi(i,k+1)
        for i=1:num_training
            temp=zeros(1,num_class);  % 1*5 %The number of the Num nearest neighbors of the ith instance which belong to the jth instance is stored in temp(1,j)
            neighbor_labels=[];
            for j=1:Num
